"""Reachable-domain policy — drives search annotation + fetch fast-skip.

Seeded from the measured egress map (DECISIONS: "Egress map"). Env-overridable so appealed domains
expand the set with ZERO code change: set AER_REACHABLE_DOMAINS to a comma-separated suffix list.

A host is "reachable" if it equals, or is a subdomain of, any entry. Anything else is treated as
known-blocked (fetch_url fast-skips it; web_search marks it). This is a heuristic over a HARD allowlist
where ~77% of probed domains reset — false negatives are rare and recorded in the miss-log for review.
Appeal-pending domains are PRE-INCLUDED here (harmless if still blocked: the fetch just fails + logs,
and starts working the moment the appeal lands).
"""
...
# Measured-reachable (2026-06-03) + appeal-pending. Lowercase, no scheme.
_DEFAULT_REACHABLE: tuple[str, ...] = (
    ...
    "google.com", "bing.com",
    # --- appeal-pending (try anyway; fails+logs until granted) ---
...
def is_reachable(url: str) -> bool:
    """True if the URL's host is in the reachable allowlist (exact or subdomain match)."""
    host = host_of(url)
    if not host:
        return False
